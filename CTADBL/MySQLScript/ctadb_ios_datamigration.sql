...
`madebchange`.`Id`,
    ...
    null,
    null,
    if(`madebchange`.`ReceiptNo` REGEXP '^-?[0-9]+$',`madebchange`.`ReceiptNo`,1) AS 'nReceiptNo',
    `madebchange`.`Email`,
    null,
...
SELECT 
	...
	`abroad`.`SaneyFormNo`,
    if(`abroad`.`ReceiptNo` REGEXP '^-?[0-9]+$',`abroad`.`ReceiptNo`,1) AS 'nReceiptNo',
	`abroad`.`Email`,
	`abroad`.`Alias`,
	null,
	null,
...
`briefgb`.`Id`,
    ...
    `briefgb`.`SaneyFormNo`,
    if(`briefgb`.`ReceiptNo` REGEXP '^-?[0-9]+$',`briefgb`.`ReceiptNo`,1) AS 'nReceiptNo',
    `briefgb`.`Email`,
	null,
	null,
	null,
...
SELECT 
    ...
    if(`ident_bookissued`.`EnteredBy` REGEXP '^-?[0-9]+$',`ident_bookissued`.`EnteredBy`,1) AS 'nEnteredBy',
	`ident_bookissued`.`Entered`,
	if(`ident_bookissued`.`EnteredBy` REGEXP '^-?[0-9]+$',`ident_bookissued`.`EnteredBy`,1) AS 'nEnteredBy'
...
SELECT `ident_note`.`NoteID`,
    ...
    `ident_note`.`Note`,
    `ident_note`.`Entered`,
    if(`ident_note`.`EnteredBy` REGEXP '^-?[0-9]+$',`ident_note`.`EnteredBy`,1) AS 'nEnteredBy'
...
WHERE sPaidUntil != '' and SUBSTRING(sPaidUntil,1,4) REGEXP '/^\d{4}$/';
...
WHERE nFormNumber != '' and nFormNumber REGEXP '^-?[0-9]+$';
