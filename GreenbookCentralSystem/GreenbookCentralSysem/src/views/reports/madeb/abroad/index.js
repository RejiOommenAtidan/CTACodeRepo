      ...
      axios.get(`/Report/GetReportCTAMadebRegionOrCountryWiseAbroad/?sMadebDisplayKey=` + madebType + `&dtRecordFrom=` + dtFrom + `&dtRecordTo=` + dtTo + `&sGroupBy=` + groupBy + `&sOrderBy=` + orderBy)
        .then(resp => {
          if (resp.status === 200) {
            ...
            setTitle(`Madeb Abroad ${grouping} Report from ${Moment(dtFrom).format(sDateFormat)} to ${Moment(dtTo).format(sDateFormat)}` );
  ...
  return (
    <>
      <Paper style={{ padding: '30px', textAlign: 'center' }} >
        <h1>Abroad Report</h1>
        <form onSubmit = {(e) => handleSubmit(abroad(e))}>
      ...
      {_.get("dtFrom.type", errors) === "required" && (
...

        {
          ...
          <MaterialTable style={{ padding: '10px', width: '100%', border: '2px solid grey', borderRadius: '10px', color: 'black', fontSize:'1.05rem' }}
            ...
            columns={columns}
            data={abroadData}
            options={{...oOptions, exportFileName: "AbroadReport"}}
            actions={[
