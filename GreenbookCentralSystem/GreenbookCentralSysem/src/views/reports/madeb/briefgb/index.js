...
export default function Report() {
  ...
  const brief = (e) => {
    ...
    if (madebType === '' || dtFrom === null || dtTo === null || orderBy === '') {
      ...
      setBackdrop(true);
      axios.get(`/Report/GetReportCTAMadebRegionOrCountryWiseBriefGB/?sMadebDisplayKey=` + madebType + `&dtRecordFrom=` + dtFrom + `&dtRecordTo=` + dtTo + `&sGroupBy=` + groupBy + `&sOrderBy=` + orderBy)
        .then(resp => {
          setBackdrop(false);
          if (resp.status === 200) {
            const grouping = orderBy === 'lstcountry.sCountry' ? 'Country Wise' : 'Region Wise'
            setTitle(`Madeb Brief GB ${grouping} Report from ${Moment(dtFrom).format(sDateFormat)} to ${Moment(dtTo).format(sDateFormat)}` );
  ...
  useEffect(() => {
    briefData.length > 0 && modifyHeaders()
  }, [briefData]);
  ...
  return (
    <>
      <Paper style={{ padding: '30px', textAlign: 'center' }} >
        <h1>Brief Report</h1>
        <form onSubmit = {(e) => handleSubmit(brief(e))}>
      ...
      {_.get("dtFrom.type", errors) === "required" && (
...

        {
          ...
          <MaterialTable style={{ padding: '10px', width: '100%', border: '2px solid grey', borderRadius: '10px', color: 'black', fontSize:'1.05rem'  }}
            ...
            data={briefData}
            options={{...oOptions, exportFileName: "BriefGBReport"}}
            actions={[
